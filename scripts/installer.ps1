...
$junk = Get-ChildItem $dir -Recurse -File | Where-Object { ($_.Name -match '\.(ttf|otf|ttc|woff2?|eot|svgz?|s?css)$') -and ($_.Name -notmatch $filter) }
...
foreach ($file in $files) {
    ...
    Copy-Item -Force -LiteralPath $file.FullName -Destination $fontDir -ErrorAction Stop
